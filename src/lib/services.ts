export type Service = {
  slug: string;
  index: string;
  title: string;
  preview: string;
  summary: string;
  body: string;
  image: string;
  detailTitle: string;
  detailIntro: string;
  detailParagraphs: string[];
};

export const services: Service[] = [
  {
    slug: "object",
    index: "01",
    title: "Object",
    preview: "Strakke bewaking voor panden, kantoren en terreinen.",
    summary: "Continue waakzaamheid voor bedrijfspanden, kantoren en terreinen.",
    body: "Objectbeveiliging bij Overview Protection staat voor continue waakzaamheid en betrouwbaarheid. Wij beschermen bedrijfspanden, kantoren en terreinen met oog voor detail en gastvrij contact, zodat veiligheid en professionaliteit altijd hand in hand gaan.",
    image: "/Object%20beveiliging.png",
    detailTitle: "Objectbeveiliging met continue waakzaamheid en betrouwbaarheid.",
    detailIntro: "Bescherming van objecten vraagt om overzicht, discipline en een aanpak die dag en nacht betrouwbaar blijft.",
    detailParagraphs: [
      "In een wereld waar veiligheid steeds belangrijker wordt, biedt objectbeveiliging een onmisbare bescherming voor uw eigendommen, personeel en bezoekers. Bij Overview Protection begrijpen we dat ieder object uniek is en vraagt om een beveiligingsaanpak op maat. Daarom zorgen onze ervaren beveiligingsprofessionals voor continue waakzaamheid en betrouwbaarheid, 24 uur per dag en 7 dagen per week.",
      "Objectbeveiliging richt zich op het beschermen van gebouwen, terreinen en andere eigendommen tegen risico’s zoals inbraak, vandalisme en onbevoegde toegang. Onze goed opgeleide beveiligers zijn alert, daadkrachtig en weten snel te handelen in het geval van een incident. Daarbij combineren wij menselijk inzicht met moderne technieken om een veilige en vertrouwde omgeving te creëren.",
      "Onze objectbeveiligers voeren uiteenlopende taken uit, waaronder toegangscontrole, regelmatige surveillancerondes, alarmopvolging en het rapporteren van bevindingen. Dankzij hun professionaliteit en klantgerichte houding stralen zij autoriteit uit, zonder de gastvrijheid uit het oog te verliezen.",
      "Kiezen voor objectbeveiliging van Overview Protection betekent kiezen voor zekerheid, flexibiliteit en maatwerk. Wij leveren oplossingen die volledig zijn afgestemd op uw situatie en staan dag en nacht klaar om uw locaties te beveiligen. Zo bent u verzekerd van een veilige omgeving waarin uw organisatie ongestoord kan functioneren.",
    ],
  },
  {
    slug: "winkelsurveillance",
    index: "02",
    title: "Winkelsurveillance",
    preview: "Zichtbare preventie en rust op de winkelvloer.",
    summary: "Zichtbare preventie met een gastvrije benadering op de winkelvloer.",
    body: "Winkelsurveillance bij Overview Protection betekent zichtbare aanwezigheid, preventie van diefstal en een gastvrije benadering richting klanten. Onze beveiligers zorgen voor een veilige winkelomgeving waarin zowel personeel als bezoekers zich prettig voelen.",
    image: "/winkel%20suilviance.png",
    detailTitle: "Winkelsurveillance met rust, overzicht en preventie.",
    detailIntro: "Voor winkels draait goede beveiliging om preventie, klantvriendelijkheid en een omgeving waarin personeel en bezoekers zich veilig voelen.",
    detailParagraphs: [
      "Veiligheid en klanttevredenheid zijn onmisbaar voor het succes van iedere winkel. Bij Overview Protection bieden wij professionele winkelsurveillance die niet alleen gericht is op het voorkomen van diefstal en vandalisme, maar ook bijdraagt aan een prettige en veilige winkelervaring. Onze ervaren beveiligers zijn zichtbaar aanwezig, signaleren risico’s vroegtijdig en zorgen ervoor dat klanten en medewerkers zich beschermd voelen.",
      "Winkelsurveillance draait om meer dan toezicht houden. Onze beveiligers combineren preventieve aanwezigheid met observatie, detectie en discreet optreden bij incidenten. Ze ondersteunen het winkelpersoneel bij lastige situaties, grijpen daadkrachtig in bij diefstal of fraude en rapporteren bevindingen met advies om de veiligheid structureel te verbeteren.",
      "Kiezen voor winkelsurveillance van Overview Protection betekent kiezen voor goed opgeleide en klantvriendelijke professionals. Wij stemmen onze beveiligingsdiensten volledig af op de specifieke behoeften van uw winkel en branche, zijn flexibel inzetbaar op drukke momenten en versterken het gevoel van veiligheid bij uw klanten. Zo creëren wij samen een winkelomgeving waarin zowel personeel als bezoekers zich op hun gemak voelen.",
    ],
  },
  {
    slug: "evenementen",
    index: "03",
    title: "Evenementen",
    preview: "Orde, controle en professionele aanwezigheid op locatie.",
    summary: "Professionele orde en veiligheid zonder afbreuk te doen aan de beleving.",
    body: "Een geslaagd evenement vraagt om veiligheid en orde. Overview Protection zorgt voor professionele evenementenbeveiliging, waarbij we risico’s voorkomen en een gastvrije sfeer behouden. Zo kunnen bezoekers zorgeloos genieten, terwijl de organisatie op ons kan rekenen.",
    image: "/evenementen%20beveiliging.png",
    detailTitle: "Evenementenbeveiliging met focus op veiligheid en beleving.",
    detailIntro: "Voor festivals, congressen, sportwedstrijden en privé-evenementen draait goede beveiliging om veiligheid, rust en professionele begeleiding.",
    detailParagraphs: [
      "Een geslaagd evenement begint met een veilige en ontspannen sfeer. Bij Overview Protection zorgen wij voor deskundige evenementbeveiliging, zodat gasten, medewerkers en artiesten zorgeloos kunnen genieten. Of het nu gaat om een festival, congres, sportwedstrijd of privéfeest: onze ervaren beveiligers creëren de omstandigheden waarin alles vlekkeloos en veilig verloopt.",
      "Evenementbeveiliging draait om het beschermen van bezoekers, deelnemers en eigendommen, terwijl er tegelijkertijd een prettige en gastvrije sfeer wordt behouden. Dit vraagt om scherpte, ervaring en uitstekende communicatieve vaardigheden. Onze beveiligers zijn speciaal getraind om professioneel en klantvriendelijk op te treden, zelfs in dynamische en drukke situaties.",
      "Wij bieden een breed scala aan diensten, waaronder toegangscontrole, publieksbegeleiding, crowdcontrol, VIP-beveiliging en directe ondersteuning bij incidenten of calamiteiten. Daarbij werken we nauw samen met organisatoren en hulpdiensten om risico’s te minimaliseren en de veiligheid van het evenement te waarborgen.",
      "Kiezen voor evenementbeveiliging van Overview Protection betekent kiezen voor ervaring, flexibiliteit en professionaliteit. Wij leveren oplossingen op maat die aansluiten bij de aard en schaal van uw evenement, zodat u zich volledig kunt richten op de organisatie en uw bezoekers onbezorgd kunnen genieten.",
    ],
  },
  {
    slug: "horeca",
    index: "04",
    title: "Horeca",
    preview: "Veilige horecagelegenheden met gastvrije uitstraling.",
    summary: "Waakzaamheid en gastvrijheid in balans voor een veilige avond.",
    body: "Horecabeveiliging draait om meer dan veiligheid alleen. Bij Overview Protection combineren we waakzaamheid met gastvrijheid. Onze beveiligers zorgen voor een veilige sfeer, terwijl gasten zich welkom en op hun gemak voelen - de perfecte balans tussen bescherming en service.",
    image: "/horeca%20beveiliging.png",
    detailTitle: "Horecabeveiliging met bescherming en service in balans.",
    detailIntro: "Voor restaurants, cafés, clubs en evenementenlocaties is een veilige omgeving onmisbaar om gastvrijheid en beleving te waarborgen.",
    detailParagraphs: [
      "De horeca draait om sfeer, gastvrijheid en beleving. Om deze kernwaarden te waarborgen, is een veilige omgeving essentieel. Bij Overview Protection bieden wij professionele horecabeveiliging, zodat u zich volledig kunt richten op uw gasten terwijl wij zorgen voor rust, orde en veiligheid in uw zaak.",
      "Horecabeveiliging richt zich op het creëren van een prettige en veilige omgeving in restaurants, cafés, clubs en evenementenlocaties. Onze beveiligers zijn getraind in het herkennen en voorkomen van risicovolle situaties en combineren dit met een gastvrije en klantvriendelijke houding. Zo dragen zij niet alleen bij aan veiligheid, maar ook aan een positieve ervaring voor uw bezoekers.",
      "Onze horecabeveiligers voeren uiteenlopende taken uit, zoals toegangscontrole, zichtbaar en gastvriendelijk toezicht, conflictbeheersing, crowdcontrol en het optreden bij incidenten of calamiteiten. Daarbij zijn zij meer dan toezichthouders: zij fungeren als ambassadeurs van uw gastvrijheid.",
      "Kiezen voor horecabeveiliging van Overview Protection betekent kiezen voor ervaren en flexibele professionals die de dynamiek van de horeca begrijpen. Met een preventiegerichte aanpak minimaliseren wij risico’s en beschermen wij de reputatie van uw onderneming. Zo kunt u uw gasten een veilige én onvergetelijke ervaring bieden.",
    ],
  },
  {
    slug: "zorg",
    index: "05",
    title: "Zorg",
    preview: "Rust, aandacht en professioneel optreden binnen zorgomgevingen.",
    summary: "Veiligheid met menselijkheid voor clienten, bezoekers en medewerkers.",
    body: "Zorgbeveiliging bij Overview Protection betekent veiligheid met oog voor menselijkheid. Onze beveiligers zijn alert en professioneel, maar ook empathisch en gastvrij, zodat clienten, bezoekers en medewerkers zich beschermd en welkom voelen.",
    image: "/zorg%20beveiliging.png",
    detailTitle: "Zorgbeveiliging met menselijkheid en professioneel overzicht.",
    detailIntro: "In zorginstellingen moet beveiliging niet alleen veiligheid brengen, maar ook aansluiten op menselijkheid, rust en de dagelijkse zorgpraktijk.",
    detailParagraphs: [
      "Een veilige zorgomgeving is de basis voor goede zorg. Bij Overview Protection bieden wij gespecialiseerde zorgbeveiliging die rust, orde en veiligheid waarborgt voor patiënten, bezoekers en zorgprofessionals. Of het nu gaat om een ziekenhuis, GGZ-instelling, verpleeghuis of verslavingskliniek, onze ervaren beveiligers zorgen voor een omgeving waarin iedereen zich beschermd voelt.",
      "Zorgbeveiliging draait om het beschermen van mensen in kwetsbare situaties en het ondersteunen van zorgteams bij spanningen, agressie of crisissituaties. Dat vraagt niet alleen om alertheid en vakkennis, maar ook om sociale intelligentie, discretie en mensgericht handelen. Onze beveiligers zijn daarom getraind om met respect en professionaliteit te opereren binnen zorginstellingen.",
      "Wij bieden diverse vormen van ondersteuning, zoals toegangsbeheer, preventieve surveillance, ondersteuning bij agressie en doortastend optreden bij calamiteiten. Daarbij combineren wij zichtbare aanwezigheid met een mensgerichte benadering, zodat veiligheid en zorg hand in hand gaan.",
      "Kiezen voor zorgbeveiliging van Overview Protection betekent kiezen voor gespecialiseerde beveiligers. Wij zorgen voor orde en veiligheid zonder de menselijkheid uit het oog te verliezen, passen onze aanpak aan de unieke behoeften van iedere zorginstelling aan en sluiten naadloos aan bij de protocollen en teamdynamiek van uw organisatie. Zo creëren we samen een veilige omgeving waarin zorgverleners zich volledig kunnen richten op hun kerntaak: het leveren van hoogwaardige zorg.",
    ],
  },
];

export function getServiceBySlug(slug: string) {
  return services.find((service) => service.slug === slug);
}